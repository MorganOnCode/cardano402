// LiveDemo.jsx — original design, bounded same-origin Worker demo transport
...
// Always call this deployment; previews must never send payments to the VPS.
...
function LiveDemo() {
  ...
  // Cooldown is local UI state; no background Worker/provider polling.
  React.useEffect(() => {
    if (!status.cooldownRemainingMs) return;
    const deadline = Date.now() + status.cooldownRemainingMs;
    const id = setInterval(() => {
      const remaining = Math.max(0, Math.ceil((deadline - Date.now()) / 1000));
      setCooldownLeft(remaining);
      if (!remaining) setStatus({ running: false, cooldownRemainingMs: 0, ready: true });
    }, 1000);
  ...
  const runDemo = async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        const res = await fetch(`${FACILITATOR_BASE}/demo/run`, {
          method: 'POST', signal: AbortSignal.timeout(210_000),
        });
        data = await res.json();
        if (!res.ok) throw new Error(data.message || data.error || `HTTP ${res.status}`);
        if (data.state !== 'pending') break;
      }
      if (data.state !== 'confirmed') {
        throw new Error(data.message || 'Confirmation is pending. No replacement payment will be created.');
      }
      // Only display completed steps after the Worker confirms the receipt.
      setSteps(STEP_LABELS_SIMPLE.map((step) => ({
        step: step.n, total: 7, label: step.tech,
        detail: data.cached ? 'Previously confirmed receipt' : 'Completed',
      })));
      setResult({ ...data, txHash: data.transaction,
        scanUrl: `https://preview.cardanoscan.io/transaction/${data.transaction}` });
      const remaining = Math.max(0, data.started + 600_000 - Date.now());
      setCooldownLeft(Math.ceil(remaining / 1000));
      setStatus({ running: false, cooldownRemainingMs: remaining, ready: true });
    } catch (e) {
    ...
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      <div>
        <div className="mono" style={{ fontSize: 11, letterSpacing: '0.1em', opacity: 0.6, marginBottom: 8, textTransform: 'uppercase' }}>
          POST /demo/run · confirmed receipt
        </div>
        <pre style={{
...
{steps.length === 0 ? <span style={{ opacity: 0.5 }}>// run the demo to see confirmed steps</span>
  : steps.map((s, i) => s ? (
  <div key={i}>
