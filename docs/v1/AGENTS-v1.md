...
- Backups: `config/config.json` holds *paths* to the signing files, so
  anything that backs up config must also back up what those keys name —
  see the Signer coverage section of `docs/backup-restore.md`. Restic
  snapshots taken before 2026-09-22 contain no `sensitive/secrets/`.
  `tests/unit/backup-signer-staging.test.ts` shows how to test
  `scripts/*.sh` from vitest: fixture tree + stub `docker`/`restic` on
  PATH + the script's `CARDANO402_*` env overrides.
